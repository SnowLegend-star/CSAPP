...
void handleRequest(int fd){
    ...
    printf("The url of request is: %s\n", url);
    //判断请求是否有效
...
cache_Item* readCache(char* url_target, int clientfd){
    ...
    for(int i=0;i<cache.cache_Item_Using;i++){
        printf("cache.cache_Set[%d].url is: %s\n",i,cache.cache_Set[i].url);
        if(strcmp(url_target,cache.cache_Set[i].url)==0){
            printf("****The item is in the cache****\n");
...
void readAndWriteResponse(int fd, rio_t* rio_Proxy2Server, char* url){
    ...
    printf("=====The content of url is: %s=====\n",url);
    // strncpy(item->url,url,sizeof(url));  西八，这个bug害得我好苦啊
    // strncpy(item->url,url,strlen(url)+1); 这个也可以
    ...
    //把这个item写入cache
    P(&W);
    printf("=========prepare to cache the item=========\n");
    cache_insert(&cache, item);
    V(&W);
    printf("The item added in the cache is: \n%s\n",cache.cache_Set[cache.cache_Item_Using-1].item_Content);
}
